...
const Collections = () => {
  ...
  const toggleSubCategory = (e) => {
    if (subCategory.includes(e.target.value)) {
      setSubCategory(prev => prev.filter(item => item !== e.target.value)); // Fixed: correct state setter
    } else {
      setSubCategory(prev => [...prev, e.target.value]);
    }
  };

  // Apply filters based on category and subcategory selection
  const applyFilter = () => {
    ...
    if (subCategory.length > 0) {
      productsCopy = productsCopy.filter(item => subCategory.includes(item.subCategory)); // Fixed key
    }
